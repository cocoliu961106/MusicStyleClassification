...
public final class FFT {
    ...
    public double[] calculateFFTMagnitude(double inputData[]) {
        ...
        for (i = 1; i < n / 2; i++) {
            mag[i] = Math.sqrt(x[i] * x[i] + x[n - i] * x[n - i]);      // FFT每个点的幅值（模）
            // System.out.println(mag[i] + " " + x[i] + " " + x[n-i]);
        }
