...
public class MFCC {
     ...
     */
    private final int m_nnumberOfFilters;       // 梅尔滤波器组的数量
    /**
     * Number of FFT points.
     */
    private final int m_nFFTLength;     // FFT采样点数
    /**
     ...
     */
    private final boolean m_oisZeroThCepstralCoefficientCalculated;     // 表示每一帧信号强度的特征
    ...
    public MFCC(int nnumberOfParameters, double dsamplingFrequency,
        ...
        if (m_oisLifteringEnabled) {        // 提升参数，让不同的倒谱系数拥有不同的权重
            // note that:
    ...
    private void initializeDCTMatrix() {
        ...
        for (int i = 0; i < m_nnumberOfParameters; i++) {
            for (int j = 0; j < m_nnumberOfFilters; j++) {
                m_ddCTMatrix[i][j] = Math.cos((i + 1.0) * (j + 1.0 - 0.5)       // DCT矩阵
    ...
    private void calculateMelBasedFilterBank(double dsamplingFrequency,
        ...
        double[] dfftFrequenciesInHz = new double[nfftLength / 2 + 1];      // 因为傅里叶转换完后的频谱是对称的
        // compute the frequency of each FFT sample (in Hz):
        double ddeltaFrequency = dsamplingFrequency / nfftLength;       // 计算每一个FFT样本的频率（横坐标）
        for (int i = 0; i < dfftFrequenciesInHz.length; i++) {
            dfftFrequenciesInHz[i] = i * ddeltaFrequency;
        }
        // convert Hz to Mel
        double[] dfftFrequenciesInMel = this.convertHzToMel(        // 转换为mel频率
        ...
        double[] dfilterCenterFrequencies = new double[nnumberofFilters + 2];       // 每个mel滤波器的中心频率
        ...
        m_nboundariesDFTBins = new int[m_nnumberOfFilters][2];      // 每一个滤波器边界[最小值，最大值]
        ...
        // allocate space
        for (int i = 0; i < nnumberofFilters; i++) {        // 每个滤波器weight的长度为每个滤波器中含有的DFT采样点个数
        ...
        for (int i = 1; i <= nnumberofFilters; i++) {
            for (int j = m_nboundariesDFTBins[i - 1][0], k = 0; j <= m_nboundariesDFTBins[i - 1][1]; j++, k++) {
                if (dfftFrequenciesInMel[j] < dfilterCenterFrequencies[i]) {
                    m_dweights[i - 1][k] = (dfftFrequenciesInMel[j] - dfilterCenterFrequencies[i - 1])      // mel滤波器公式求Hm(k)
     ...
     */
    public double[] getParameters(double[] fspeechFrame) {      // 这里开始获取MFCC
        ...
        for (int i = 0; i < m_nnumberOfFilters; i++) {
            ...
            if (m_ousePowerInsteadOfMagnitude) {
                ...
                double[] fmagnitudeSpectrum = m_fft     // Xa(k)
            ...
            // ISIP (Mississipi univ.) implementation       取对数（大于等于最小值）
