...
object TrainExampleCNN {

  def main(args: Array[String]) {
    //1 构建Spark对象
    val conf = new SparkConf().setMaster("local[4]").setAppName("Mnisttrain")
    ...
    // 将模型序列化保存在文件中
    val save_path = "C:\\Users\\1-49\\Desktop\\spark实验测试数据\\MlibTest\\CNNmodel.obj"
    Serialization.serialize_file(CNNmodel, save_path)
    println("模型保存成功!")
  }
}
