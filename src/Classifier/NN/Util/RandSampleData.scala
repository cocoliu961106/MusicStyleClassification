...
import breeze.numerics.{cos => Bcos}
...
object RandSampleData extends Serializable {
  ...
  def RandM(
             ...
             b1: Double,
             b2: Double,
             function: String): BDM[Double] = {   // 返回值格式为 BDM(Label, 样本数据(一行代表一个样本， 一列代表特征的维数))
    //    val n1 = 2
    //    val n2 = 3
    //    val b1 = -30
    //    val b2 = 30
    val bdm1 = BDM.rand(n1, n2) * (b2 - b1).toDouble + b1.toDouble    // n1 × n2数值在 b1 - b2 之间的随机矩阵
    val bdm_y = function match {
      case "rosenbrock" =>
        val xi0 = bdm1(::, 0 to (bdm1.cols - 2))    // bdm1除最后一列
        val xi1 = bdm1(::, 1 to (bdm1.cols - 1))    // bdm1全部行列
        val xi2 = (xi0 :* xi0)                      // 对应位置平方
