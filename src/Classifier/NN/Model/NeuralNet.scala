...
/**
  ...
  */
case class NNConfig(
                     size: Array[Int], //神经网络结构
                     layer: Int, //神经网络层数
                     activation_function: String, //隐含层函数
                     learningRate: Double, //学习率 α
                     momentum: Double, //Momentum因子  x = x - lr*dx + v * momentum (其中，v = -lr+dx (dx是上一次迭代的导数))  作用是加速迭代
                     scaling_learningRate: Double, //学习迭代因子
                     weightPenaltyL2: Double, //正则化因子 w求偏导的后面一项，作用是减小过拟合
                     nonSparsityPenalty: Double, //权重稀疏度惩罚因子
                     sparsityTarget: Double, //权重稀疏度目标值
                     inputZeroMaskedFraction: Double, //权重加入噪声因子
                     dropoutFraction: Double, //Dropout因子（随机临时使得一些节点输出为零）
                     testing: Double,
                     output_function: String //输出层激活函数
...
class NeuralNet(
  ...
  def NNtrain(train_d: RDD[(BDM[Double], BDM[Double])], opts: Array[Double]): NeuralNetModel = {
    ...
    var n = 0
    var loss_train_e = Array.fill(numepochs)(0.0) // 训练误差
    var loss_val_e = Array.fill(numepochs)(0.0) // 交叉验证误差
    // numepochs是循环的次数
    for (i <- 1 to numepochs) {
      initStartTime = System.currentTimeMillis()
      val splitW2 = Array.fill(numbatches)(1.0 / numbatches)
      // 根据分组个数样划分样本，每组样本个数相等
...
object NeuralNet extends Serializable {
  ...
  def InitialWeight(size: Array[Int]): Array[BDM[Double]] = {
    ...
    // nn.vW{i - 1} = zeros(size(nn.W{i - 1}));
    val n = size.length
    val nn_W = ArrayBuffer[BDM[Double]]() // 创建空ArrayBuffer
    for (i <- 1 to n - 1) {
  ...
  /**
    * relu激活函数
    */
  def lRelu(matrix: BDM[Double]): BDM[Double] = {
  ...
  def softmax(matrix: BDM[Double]): BDM[Double] = {
    ...
    for (i <- 0 until result.rows) {
      val sum = Bsum(ematrix(i, ::))
      for (j <- 0 until result.cols) {
        result(i, j) = ematrix(i, j) / sum
      }
    }
    result
  }
  ...
  def NNff(
            ...
            bc_nn_W: org.apache.spark.broadcast.Broadcast[Array[BDM[Double]]]): RDD[(NNLabel, Array[BDM[Double]])] = { //输出格式为(NNLabel(label, nn_a, error), dropOutMask)
    // 第1层:a(1)=[1 x]
    // 增加偏置项b
    ...
    val train_data3 = train_data2.map { f =>
      ...
      val nnan1 = bc_config.value.output_function match {
        ...
        case "linear" =>
          ...
          awn2
        case "softmax" => {
          val awn2 = NeuralNet.softmax(awn1)
          awn2
        }
      }
      nn_a += nnan1
  ...
  def ActiveP(
    ...
    for (i <- 1 to bc_config.value.layer - 1) {
      val pi1 = train_nnff.map(f => f._1.nna(i)) //每个样本每层节点的输出
      ...
      val (piSum, miniBatchSize) = pi1.treeAggregate((initpi, 0L))(
        seqOp = (c, v) => {
          ...
          var nna2: BDM[Double] = v
          if (i != bc_config.value.layer - 1) { // 除输出层外,每层的权重要去掉偏置节点
            nna2 = v(::, 1 to -1)
          } else {
            nna2 = v
          }
      ...
      val piAvg = piSum / miniBatchSize.toDouble // //每层节点的sparsity平均值
  ...
  def NNbp(
    ...
    val train_data5 = train_nnff.map { f =>
      ...
      dn += nndn
      (f._1, f._2, dn) // ((样本标签，每一层输出，输出误差)， dropout， 输出层反向传播的偏导)
    }
      ...
      for (i <- bc_config.value.layer - 2 to 1 by -1) {
        ...
        val nnd_act = bc_config.value.activation_function match {
          ...
          case "tanh_opt" =>
            ...
            d_act
          case "lrelu" =>
            val tmp_bdm = new BDM[Double](nn_a(i).rows, nn_a(i).cols)
            for (k <- 0 until tmp_bdm.rows) {
        ...
        val sparsityError = if (bc_config.value.nonSparsityPenalty > 0) {
          val nn_pi1 = bc_nn_p.value(i) // 第i层的平均稀疏度
          ...
          val sparsity = BDM.horzcat(Bm1, nn_pi2 * bc_config.value.nonSparsityPenalty) // [1 稀疏度惩罚误差*稀疏度惩罚因子]
          sparsity
        } else {
  ...
  def NNapplygrads(
    ...
    for (i <- 0 to bc_config.value.layer - 2) {
      ...
      val nndwi2 = nndwi :* bc_config.value.learningRate // α* [dw + λw]
